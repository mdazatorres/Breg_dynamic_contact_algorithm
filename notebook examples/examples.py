...
class Example6: # quartic function 2
    def __init__(self, n):
        self.n = n
        self.p0 = np.zeros(self.n)
...
class Example7:  # Quartic function
...
class Example8: # Schwefel function:
...
class Example9:  # Matyas function
    def __init__(self):
        self.n = 2
        # self.seed = seed
        # x0= np.array([10, -7])
...
class Example10:  # Levi function
    ...
    def gradf(self, x):
        ...
        gradx = (-2*x + 2) * np.cos(3*y*np.pi)**2 + 6*np.sin(3*x*np.pi)*np.pi*np.cos(3*x*np.pi) + 4*x - 4
        grady = 96*np.pi*np.sin(y*np.pi)*(x - 1)**2*np.cos(y*np.pi)**5 + (-8*y + 8)*np.cos(y*np.pi)**4 - 96*np.pi*np.sin(y*np.pi)*(x**2 - 1/6*y**2 - 2*x + 1/3*y + 5/6)*np.cos(y*np.pi)**3 \
               + (8*y - 8)*np.cos(y*np.pi)**2 + 18*np.pi*np.sin(y*np.pi)*(x - 5/3 + (2*y)/3)*(x - 1/3 - (2*y)/3)*np.cos(y*np.pi) + 2*y - 2
        return np.array([gradx, grady])

    def f2D(self, x1, x2):
        return np.sin(3*np.pi*x1)**2 + (x1-1)**2 * (1 + np.sin(3 * np.pi * x2)**2) +\
               (x1-1)**2 * (1 + np.sin(2 * np.pi * x2)**2)



class Example11:  # sum of square
    def __init__(self, n=100):
        self.n = n
        self.p0 = np.zeros(self.n)
        self.x0 = 10 * np.ones(self.n)
        self.s0 = 0
        self.t0 = 0

    def f(self, x):
        seq = np.arange(1, self.n + 1)
        return np.dot(seq, x ** 2)
...
class Example12:  # Beale function
    def __init__(self):
        self.n = 2
        # self.seed = seed
        # x0= np.array([10, -7])
...
class Example13:  # Chung.Reynolds funcion
    def __init__(self, n=50):
        self.n = n
        self.p0 = np.zeros(self.n)
...
class Example15:  # Qing function
    ...
    def f2D(self, x1, x2):
        return ((x1 ** 2 -1) + (x2 ** 2 -2))**2


class Example16:  # Three-hump
    def __init__(self):
        self.n = 2
        # self.seed = seed
        # x0= np.array([10, -7])
