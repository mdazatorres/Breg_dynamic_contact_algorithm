...
from integratorsV1 import CM, NAG, RGD, CRGD, CNAG
from example_diabetes_LRV1 import gradLp, Lp, loss_func_test, n, Xvalid, Yvalid
...
def tuning_process(num_trials, max_ite, N, method):
    ...
    for k in range(num_trials):
        ...
        if method ==CM:
            ...
            Dt = np.linspace(1e-4, 1e-3, N)
            Mu = np.linspace(0.8, 0.99, N)
            M = np.linspace(1e-6, 1e-3, N)
            Vc = np.linspace(1e+3, 1e+6, N)
            ...
            Dt = np.linspace(1e-3, 1e-1, N)
            Mu = np.linspace(0.1, 0.6, N)
